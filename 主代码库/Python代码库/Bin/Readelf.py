  ...
  '''
    ...
    if ei_class == 1:
        ...
        e_phoff, e_shoff, e_flags, e_ehsize, e_phentsize, e_phnum, e_shentsize, e_shnum, e_shstrndx = struct.unpack(
            'IIIHHHHHH', elf.read(24))
    else:
        ei_entry = struct.unpack('Q', elf.read(8))[0]
        e_entry = ei_entry
        e_phoff, e_shoff, e_flags, e_ehsize, e_phentsize, e_phnum, e_shentsize, e_shnum, e_shstrndx = struct.unpack(
            'QQIHHHHHH', elf.read(32))
    print("ELF Header:")
    print("Magic:                             %02x %02x %02x %02x %02x %02x %02x %02x" % (
    ...
    if ei_class == 1:
        sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size, sh_link, sh_info, sh_addralign, sh_entsize = struct.unpack(
            'IIIHHIIIII', elf.read(36))
    else:
        sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size, sh_link, sh_info, sh_addralign, sh_entsize = struct.unpack(
            'IIQQQQIIQQ', elf.read(64))
    ...
    print("%10s 0x%10s 0x%14s 0x%14s 0x%10s 0x%10s %010s" % (
    ...
    for i in range(0, e_phnum):
        ...
        print("%10s 0x%08x 0x%014x 0x%014x 0x%010x 0x%010x %010s" % (
        PT_TYPE[p_type] if p_type in PT_TYPE else p_type, p_offset, p_vaddr, p_paddr, p_filesz, p_memsz,
        PT_FLAGS[p_flags]))
    ...
    "Name", "Type", "Address", "Offset", "Size", "EntSize", "Flag", "Link", "Info", "Align"))
    for i in range(0, e_shnum):
        ...
        if sh_name in string_table:
            ...
            i, string_table[sh_name], SH_TYPE[sh_type] if sh_type in SH_TYPE else sh_type, sh_addr, sh_offset, sh_size,
            ...
            print("[%02d]%20s%15s%10x%10d%8d%8d%5s%5s%5s%6s" % (
            i, sh_name, SH_TYPE[sh_type] if sh_type in SH_TYPE else sh_type, sh_addr, sh_offset, sh_size, sh_entsize, f,
    ...
    if e_shdynsym >= 0 and e_shdynstr >= 0:
        ...
        for i in range(0, sh_size / 24):
            if ei_class == 1:
                st_name, st_info, st_other, st_shndx, st_value, st_size = struct.unpack('IIIBBH', dynsym_section[
                                                                                                  i * 16:(i + 1) * 16])
            else:
                st_name, st_info, st_other, st_shndx, st_value, st_size = struct.unpack('IBBHQQ', dynsym_section[
                                                                                                  i * 24:(i + 1) * 24])
            ...
            if st_name in dynsymbol_table:
                print("%04d%10d%10d%10s%10s%10s%10d%30s" % (i, st_value, st_size, STT_TYPE[ELF_ST_TYPE(st_info)],
                                                            ...
                                                            STV_VISIBILITY[ELF_ST_VISIBILITY(st_other)], st_shndx,
                                                            dynsymbol_table[st_name],))
            else:
    ...
    if e_shsymndx >= 0 and e_shstrndx >= 0:
        ...
        for i in range(0, sh_size / 24):
            if ei_class == 1:
                st_name, st_info, st_other, st_shndx, st_value, st_size = struct.unpack('IIIBBH', sym_section[
                                                                                                  i * 16:(i + 1) * 16])
            else:
                st_name, st_info, st_other, st_shndx, st_value, st_size = struct.unpack('IBBHQQ', sym_section[
                                                                                                  i * 24:(i + 1) * 24])
